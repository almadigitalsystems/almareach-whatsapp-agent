...
const HANDOFF_KEYWORDS = ['speak to someone', 'human', 'agent', 'speak to a person', 'talk to someone', 'real person', 'hablar con alguien', 'persona real', 'representante'];
...
async function sendHandoffSMS(from, lastMessage) {
  const phones = (process.env.NOTIFICATION_PHONES || '').split(',').map(p => p.trim()).filter(Boolean);
  if (phones.length === 0) return;

  const twilioClient = twilio(process.env.TWILIO_ACCOUNT_SID, process.env.TWILIO_AUTH_TOKEN);
  const businessName = process.env.BUSINESS_NAME || 'AlmaReach';
  const customerPhone = from.replace('whatsapp:', '');
  const preview = lastMessage.length > 100 ? lastMessage.substring(0, 100) + '...' : lastMessage;

  for (const phone of phones) {
    try {
      await twilioClient.messages.create({
        body: `[${businessName} WhatsApp] Human handoff requested.\nCustomer: ${customerPhone}\nMessage: ${preview}\nPlease follow up.`,
        messagingServiceSid: process.env.MESSAGING_SERVICE_SID,
        to: phone,
      });
      console.log(`[sms] Handoff notification sent to ${phone}`);
    } catch (err) {
...
async function sendHandoffEmail(from, conversationHistory) {
  try {
    ...
    if (!process.env.NOTIFICATION_EMAIL_PASSWORD) {
      console.log('[email] Skipping email notification - no NOTIFICATION_EMAIL_PASSWORD set');
      return;
    }
    ...
    console.log('[email] Handoff email sent');
  } catch (err) {
...
app.post('/whatsapp/webhook', async (req, res) => {
  ...
  if (isHandoffRequest(body)) {
    ...
    await Promise.all([
      sendHandoffSMS(from, body),
      sendHandoffEmail(from, history),
    ]);
